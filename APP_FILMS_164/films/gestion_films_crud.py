    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_dinosaure (id_dinosaure,nom_dinosaure) VALUES (NULL,%(value_nom_film)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            str_sql_update_nom_film = """UPDATE t_dinosaure SET nom_dinosaure = %(value_nom_film)s,
                                                            type_apprivoisement_dinosaure = %(value_description_film)s,
                                                            WHERE id_dinosaure = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_dinosaure WHERE id_dinosaure = %(value_id_film)s"
            ...
            print("data_film ", data_film, " type ", type(data_film), " genre ",
                  data_film["nom_dinosaure"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "film_update_wtf.html"
            form_update_film.nom_film_update_wtf.data = data_film["nom_dinosaure"]
            form_update_film.description_film_update_wtf.data = data_film["type_apprivoisement_dinosaure"]
            # Debug simple pour contrôler la valeur dans la console "run" de PyCharm
            print(f" type d'apprivoisement  ", data_film["type_apprivoisement_dinosaure"], "  type ", type(data_film["type_apprivoisement_dinosaure"]))
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_conf_del_film.data:
            ...
            flash(f"Effacer le dinosaure de façon définitive de la BD !!!", "danger")
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_personne_apprivoise_dinosaure WHERE fk_dinosaure = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_dinosaure WHERE id_dinosaure = %(value_id_film)s"""
            ...
            flash(f"Dinosaure définitivement effacé !!", "success")
            print(f"Dinosaure définitivement effacé !!")
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_dinosaure WHERE id_dinosaure = %(value_id_film)s"""
