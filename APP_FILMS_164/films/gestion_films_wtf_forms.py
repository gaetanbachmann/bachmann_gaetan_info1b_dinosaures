...
class FormWTFAddFilm(FlaskForm):
    ...
    """
    nom_film_regexp = ""
    nom_film_add_wtf = StringField("Nom du dinosaure ", validators=[Length(min=2, max=2000, message="min 2 max 20"),
    ...
    submit = SubmitField("Enregistrer dinosaure")
...
class FormWTFUpdateFilm(FlaskForm):
    ...
    description_film_update_wtf = StringField("Type d'apprivoisement ", widget=TextArea())
    cover_link_film_update_wtf = StringField("Lien de l'image du dinosaure", widget=TextArea())
    datesortie_film_update_wtf = DateField("Date de sortie du film", validators=[InputRequired("Date obligatoire"),
                                                                                 DataRequired("Date non valide")])
    submit = SubmitField("Update dinosaure")
...
class FormWTFDeleteFilm(FlaskForm):
    """
        ...
        submit_btn_conf_del : Bouton de confirmation pour effacer un "dinosaure".
        submit_btn_annuler : Bouton qui permet d'afficher la table "t_dinosaure".
    """
    nom_film_delete_wtf = StringField("Effacer ce dinosaure")
    submit_btn_del_film = SubmitField("Effacer dinosaure")
