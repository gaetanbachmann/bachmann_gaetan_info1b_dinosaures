    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_dinosaure, nom_dinosaure, type_apprivoisement_dinosaure,
                                                            GROUP_CONCAT(prenom_personne) as Propriete FROM t_personne_apprivoise_dinosaure
                                                            RIGHT JOIN t_dinosaure ON t_dinosaure.id_dinosaure = t_personne_apprivoise_dinosaure.fk_dinosaure
                                                            LEFT JOIN t_personne ON t_personne.id_personne = t_personne_apprivoise_dinosaure.fk_personne
                                                            GROUP BY id_dinosaure"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_dinosaure= %(value_id_film_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    flash("""La table "t_dinosaure" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_film_sel > 0:
                    # Si l'utilisateur change l'id_film dans l'URL et qu'il ne correspond à aucun film
                    flash(f"Le dinosaure {id_film_sel} demandé n'existe pas !!", "warning")
                else:
                    flash(f"Données dinosaures et personnes affichées !!", "success")
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_personne, prenom_personne FROM t_personne ORDER BY id_personne ASC"""
            ...
            lst_data_film_selected = [item['id_dinosaure'] for item in data_genre_film_selected]
            ...
            lst_data_genres_films_non_attribues = [item['prenom_personne'] for item in data_genres_films_non_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_personne_apprivoise_dinosaure (id_personne_apprivoise_dinosaure, fk_personne, fk_dinosaure)
            ...
            strsql_delete_genre_film = """DELETE FROM t_personne_apprivoise_dinosaure WHERE fk_personne = %(value_fk_genre)s AND fk_dinoasure = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_dinosaure, nom_dinosaure, type_apprivoisement_dinosaure, GROUP_CONCAT(prenom_personne) as Propriete FROM t_personne_apprivoise_dinosaure
                                        INNER JOIN t_dinosaure ON t_dinosaure.id_dinosaure = t_personne_apprivoise_dinosaure.fk_dinosaure
                                        INNER JOIN t_personne ON t_personne.id_personne = t_personne_apprivoise_dinosaure.fk_personne
                                        WHERE id_dinosaure = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_personne, prenom_personne FROM t_personne WHERE id_personne not in(SELECT id_personne as id_personne_apprivoise_dinosaure FROM t_personne_apprivoise_dinosaure
                                                    INNER JOIN t_dinosaure ON t_dinosaure.id_dinosaure = t_personne_apprivoise_dinosaure.fk_dinosaure
                                                    INNER JOIN t_personne ON t_personne.id_personne = t_personne_apprivoise_dinosaure.fk_personne
                                                    WHERE id_dinosaure = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_dinosaure, id_personne, prenom_personne FROM t_personne_apprivoise_dinosaure
                                            INNER JOIN t_dinosaure ON t_dinosaure.id_dinosaure = t_personne_apprivoise_dinosaure.fk_dinosaure
                                            INNER JOIN t_personne ON t_personne.id_personne = t_personne_apprivoise_dinosaure.fk_personne
                                            WHERE id_dinosaure = %(value_id_film_selected)s"""
